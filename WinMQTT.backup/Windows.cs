...
namespace WinMQTT
{
    ...
    {
        private static readonly MMDevice DefaultAudioDevice =
        ...
        private static void ChangeVolume(string volume)
        {
            try
            {
                float.TryParse(volume, out var v);
                DefaultAudioDevice.AudioEndpointVolume.MasterVolumeLevelScalar = v / 100;
            }
            catch (Exception e)
            {
                Debug.WriteLine($"Error = {e}");
        ...
        private static string GetVolume()
        {
            try
            {
                Volume = Math.Round(DefaultAudioDevice.AudioEndpointVolume.MasterVolumeLevel * 100);
            }
            catch (Exception e)
            {
